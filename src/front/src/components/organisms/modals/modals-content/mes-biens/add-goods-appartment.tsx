...
export default function AddGoodsAppartmentModal() {
  // All hooks must be at the top
  const { toggle, data } = useModalStore();
  const { user } = useAuthStore();
  const { addAppartment, updateAppartment } = usePropertiesStore();
  const [customType, setCustomType] = useState("");

  const photoRef = useRef<HTMLInputElement>(null);

  const { handleSubmit, control, reset, getValues, watch, setValue } =
    useForm<IAppartmentForm>({
      defaultValues: {
        name: "",
        typeOfAppartment: "",
        city: "",
  ...
  const typeOfAppartment = watch("typeOfAppartment");
  ...
  const { tenants: tenantsData } = useGetEveryTenants();

  // Compute values after hooks
  const isEditMode = data?.type === "edit";
  const apartmentToEdit = data?.apartment as Appartment | undefined;

  console.log("apartment to edit", apartmentToEdit);
  ...
  useEffect(() => {
    if (tenantsData.length > 0 && properties.length > 0) {
      setDataLoaded(true);
    }
  }, [tenantsData.length, properties.length]);

  // Pre-fill form when in edit mode
  useEffect(() => {
    if (isEditMode && apartmentToEdit && dataLoaded) {
      // Use setTimeout to ensure setValue happens after component is fully mounted
      const timer = setTimeout(() => {
        setValue("name", apartmentToEdit.name);
        setValue("city", apartmentToEdit.city);
        setValue("address", apartmentToEdit.address);
        setValue("code", apartmentToEdit.code);
        setValue("amount", apartmentToEdit.price.toString());
        setValue("cautionAmount", apartmentToEdit.cautionAmount.toString());
        setValue("startLocation", new Date(apartmentToEdit.dateDebutLocation));
        setValue("endLocation", new Date(apartmentToEdit.dateFinLocation));
        setValue("deadlinePaiement", apartmentToEdit.dateLimitePaiement);
        setValue("tenant", apartmentToEdit.tenantsId);
        setValue("building", apartmentToEdit.buildingsId || "");

        setValue("penaltyEnabled", apartmentToEdit.penaltyEnabled || false);
        setValue(
          "penaltyPercentage",
          apartmentToEdit.penaltyPercentage?.toString() || ""
        );
        // Handle typeOfAppartment setting
        const standardTypes = appartmentTypes.slice(0, -1); // Get all types except "Autre"
        const apartmentType = apartmentToEdit.typeOfAppartment;
        if (standardTypes.includes(apartmentType)) {
          // If it's a standard type, set it directly
          setValue("typeOfAppartment", apartmentType);
          setCustomType(""); // Reset custom type
        } else {
          // If it's a custom type, set as "Autre" and populate custom field
          setValue("typeOfAppartment", "Autre");
          setCustomType(apartmentType);
        }
      }, 100);
  ...
  const onSubmit: SubmitHandler<IAppartmentForm> = async (value) => {
    // console\.log\([^)]*\);?\s*
    if (!user) {
    ...
    // Determine the number of months between startLocation and endLocation
    const months =
      (value.endLocation.getFullYear() - value.startLocation.getFullYear()) *
        12 +
      (value.endLocation.getMonth() - value.startLocation.getMonth());
    ...
    const payload = {
      ...
      code: value.code,
      // extract "T1", "T2", ... when present, otherwise keep the original value
      type_of_apartment: (() => {
        const raw = String(value.typeOfAppartment || "");
        // If user provided a custom value prefixed with "Autre: ", return the custom part
        if (raw.startsWith("Autre: ")) return raw.substring(7);
        // Otherwise try to extract the leading "T<number>"
        const m = raw.match(/^T\d+/);
        return (m ? m[0] : raw).toLowerCase();
      })(),
      caution_amount: parseInt(value.cautionAmount),
      price: parseFloat(value.amount),
      state_photos: photo,
      real_estate_entities_id: user.id,
      buildings_id: value.building === "" ? "" : value.building,
      ...
      duree_location: months,
      date_limite_paiement: value.deadlinePaiement,

      penalty_enabled: value.penaltyEnabled,
      penalty_percentage: value.penaltyEnabled
        ? parseFloat(value.penaltyPercentage)
        : null,
    };
    ...
    try {
      if (isEditMode && apartmentToEdit) {
        ...
        const { data: responseData, error } =
          await propertyProvider.updateAppartment(apartmentToEdit.id, payload);

        // console\.log\([^)]*\);?\s*

        if (responseData && responseData.appartment) {
          toast.success("Appartement modifié avec succès");
          updateAppartment(responseData.appartment);
        } else {
        ...
        const { data: responseData, error } =
          await propertyProvider.createAppartment(payload);

        if (responseData) {
          toast.success(responseData.message);
          addAppartment(responseData.appartment);
        } else {
  ...
  const checkForm = useCallback(
    (step: number) => {
      ...
      if (step === 1) {
        if (!!(name && city && typeOfAppartment && address && code && tenant)) {
          return true;
        }
  ...
  return (
    <section className="w-full p-8 flex flex-col h-[74vh] max-h-[74vh]">
      <div className="w-full flex items-center justify-between">
        <DialogTitle>
          {isEditMode ? "Modifier l'appartement" : "Ajouter un bien"}
        </DialogTitle>
      ...
      >
            ...
            >
                ...
                render={({ field }) => (
                  <Select onValueChange={field.onChange} value={field.value}>
                    <SelectTrigger className="h-12">
                      <SelectValue placeholder="Selectioner une ville" />
                    </SelectTrigger>
                    <SelectContent>
                      {villes.map((ville) => (
                  ...
                  render={({ field }) => (
                    <>
                      <Select
                        ...
                        // Show "Autre" in select when we have a custom type
                        value={
                      ...
                      >
                        <SelectTrigger className="h-12">
                          <SelectValue placeholder="Quel est le type de appartement ?" />
                        </SelectTrigger>
                        <SelectContent>
                          {appartmentTypes.map((value) => (
                      ...
                      {/* Show input when value is "Autre" OR starts with "Autre:" */}
                      {(field.value === "Autre" ||
                        field.value.startsWith("Autre:")) && (
                        <div className="mt-2">
                          <MUIInput
                            name="otherTypeOfAppartment"
                            value={customType}
                            onChange={(e) => {
                              const newValue = e.target.value;
                              setCustomType(newValue);
                              // Only update the field value when we have actual input
                              field.onChange(
              ...
              <div className="mt-2">
                <Controller
                  ...
                  render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger className="h-12 mt-0">
                  ...
                  render={({ field }) => (
                    <>
                      ...
                      >
                        ...
                        <SelectContent>
                          {Array.from({ length: 29 }, (_, i) => i + 1).map(
                            (day) => (
                    ...
                    <Controller
                      ...
                      rules={{
                        required: penaltyEnabled
                          ? "Le pourcentage de pénalité est requis"
                          : false,
                        min: {
                          value: 0,
        ...
        <div className="mt-8 w-full flex items-center justify-between  bg-white pt-4 z-10">
          <Button
