...
import { CreateLocalDto } from "@/api/properties/dto";
...
export default function AddGoodsLocalModal() {
  const { toggle, data } = useModalStore();
  const { user } = useAuthStore();
  const [customType, setCustomType] = useState("");

  const { addLocal, updateLocal } = usePropertiesStore();
  ...
  const localToEdit = data?.local as Local | undefined;
  ...
  }, [tenants.length, properties.length]);
  ...
  useEffect(() => {
    if (isEditMode && localToEdit && dataLoaded) {
      const timer = setTimeout(() => {
        setValue("name", localToEdit.name);
        setValue("city", localToEdit.city);
        setValue("address", localToEdit.address);
        setValue("code", localToEdit?.code);
        setValue("amount", localToEdit?.price?.toString());
        setValue("dureeLocation", localToEdit.dureeLocation.toString());
        setValue("cautionAmount", localToEdit.cautionAmount.toString());
        setValue("startLocation", new Date(localToEdit.dateDebutLocation));
        setValue("endLocation", new Date(localToEdit.dateFinLocation));
        setValue("deadlinePaiement", localToEdit.dateLimitePaiement);
        setValue("tenant", localToEdit.tenantsId);
        setValue("building", localToEdit.buildingsId || "default");
        setValue("typeOfLocal", localToEdit.typeOfLocal || "shop");
        setValue("penaltyEnabled", localToEdit.penaltyEnabled || false);
        setValue(
          "penaltyPercentage",
          localToEdit.penaltyPercentage?.toString() || ""
        );
      }, 100);
  ...
  }, [isEditMode, localToEdit, dataLoaded, setValue, reset, filterBuilding]);
  ...
  const onSubmit: SubmitHandler<ILocalForm> = async (value) => {
    if (!user) {
    ...
    const payload = {
      ...
      code: value.code,
      type_of_local: value.typeOfLocal,
      caution_amount: parseInt(value.cautionAmount, 10),
      montant_loyer: parseFloat(value.amount),
      state_photos: photo,
      ...
      penalty_percentage: value.penaltyEnabled
        ...
        : null,
    } satisfies CreateLocalDto;

    try {
      if (isEditMode && localToEdit) {
        // Update existing local
        const { data: responseData, error } =
          await propertyProvider.updateLocal(localToEdit.id, payload);

        if (responseData && responseData.local) {
          toast.success("Local modifié avec succès");
          updateLocal(responseData.local);
        } else {
        ...
        // Create new local
        const { data: responseData, error } =
          await propertyProvider.createLocal(payload);
  ...
  return (
    <section className="w-full p-8 flex flex-col h-[80vh] max-h-[80vh]">
      <div className="w-full flex items-center justify-between">
        <DialogTitle>
          {isEditMode ? "Modifier le local" : "Ajouter un bien"}
        </DialogTitle>
            ...
            >
              ...
              <div className="mt-2">
                <Controller
                  ...
                  control={control}
                  rules={{
                    required: "Le nom du local est requis",
                  }}
                  render={({ field }) => (
                    <MUIInput
                      {...field}
              ...
              <div className="mt-2">
                <Controller
                  ...
                  render={({ field }) => (
                    <>
                      <label>Selectionner un immeuble</label>
                      <Select
